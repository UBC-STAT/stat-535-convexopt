...
$$\nabla g(u) = \frac{\partial}{\partial y} \min_x L(x,u) = A x^{*} - b$$
...
The above is the "standard" form, but the intuition is as follows:
...
$$\begin{aligned}
y^{(k)}&=\arg\min_y h(y) +  (u^{(k-1)})^\top y + \frac{\rho}{2}\norm{y-z^{(k-1)} + u^{(k-1)}}_2^2\\
z^{(k)}&=\arg\min_z  g(z) +  (u^{(k-1)})^\top z + \frac{\rho}{2}\norm{y^{(k-1)}-z + u^{(k-1)}}_2^2\\
u^{(k)} &= u^{(k-1)}+\rho(y^{(k)}-z^{(k)})\\
\end{aligned}$$
